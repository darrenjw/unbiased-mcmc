...
\item Possible to engineer early coupling (via initial conditions and MH kernel, for example), but this might be a dangerous strategy (see multimodal example in paper)
\end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
    ...
    \item The reflection coupling is really designed for high dimensional Gaussian kernels, where it encourages strong positive correlation between generated values
      \item In the univariate case, it's not very clear why the reflection coupling isn't slightly worse than the standard coupling
  \end{itemize}
  }
...
\frame{
  ...
  \item In any case, any non-zero $k$ limits speed-up of parallel coupled chains relative to ``one long run'', via Amdahl's law for parallel chains MCMC (Wilkinson, 2005):
