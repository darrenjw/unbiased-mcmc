...
object UnbiasedMcmc {
  ...
  def metKernel[T](q: T => ContinuousDistr[T])(logPi: T =>
    Double)(x: T): Rand[T] = for {
    xs <- q(x)
      u <- Uniform(0,1)
...
object CouplingExamples {
  ...
  def metropTest: Unit = {
    val chain = Stream.iterate(0.0)(xi => metKernel(
      (x: Double) => Uniform(x-0.5, x+0.5))(
      x => Gaussian(0.0, 1.0).logPdf(x))(xi).draw
    ).
      drop(1).
      take(10000).
      toArray
  ...
  def coupledMetropTest: Unit = {
    val chainProcess = Stream.iterate((5.0,-5.0))(xi =>
